class Type < ActiveRecord::Base
  ...
  # SELECT * FROM 
			...
			#GROUP BY TypeName
  scope :type_on_subject, lambda {|arg| select('DISTINCT Types.ID, Types.Name').\
                                            ...
                                            where(["rt.TypeAssigned = ? AND rs.OtherResourceList = ? AND rs.SubjectID = ? ", "Y", "Y","#{arg}"]) unless arg.nil? }
